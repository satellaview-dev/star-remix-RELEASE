...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (isStoryMode)
		{
			detailsText = "Currently in: Week " + storyWeek;
		}
		else
		{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'starpeebo':
				dialogue = CoolUtil.coolTextFile(Paths.txt('starpeebo/starpeeboDialogue'));
			case 'fruity':
				dialogue = CoolUtil.coolTextFile(Paths.txt('fruity/fruityDialogue'));
			case 'bruh-battle':
				dialogue = CoolUtil.coolTextFile(Paths.txt('bruh-battle/bruh-battleDialogue'));
			case 'senvibe':
				dialogue = CoolUtil.coolTextFile(Paths.txt('senvibe/senvibeDialogue'));
			case 'mean-flowers':
				dialogue = CoolUtil.coolTextFile(Paths.txt('mean-flowers/mean-flowersDialogue'));
			case 'ouchies':
				dialogue = CoolUtil.coolTextFile(Paths.txt('ouchies/ouchiesDialogue'));
		}
		...
		switch(SONG.stage)
		{
			...
			case 'roses':
			{
					...
					var bgSky = new FlxSprite().loadGraphic(Paths.image('weeb/rosesSky','week6'));
					bgSky.scrollFactor.set(0.1, 0.1);
					add(bgSky);

					var repositionShit = -200;

					var bgSchool:FlxSprite = new FlxSprite(repositionShit, 0).loadGraphic(Paths.image('weeb/rosesSchool','week6'));
					bgSchool.scrollFactor.set(0.6, 0.90);
					add(bgSchool);

					var bgStreet:FlxSprite = new FlxSprite(repositionShit).loadGraphic(Paths.image('weeb/rosesStreet','week6'));
					bgStreet.scrollFactor.set(0.95, 0.95);
					add(bgStreet);
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer)
		{
			...
			else
			{
				if (dialogueBox != null)
				{
					...
					if (SONG.song.toLowerCase() == 'ouchies')
					{
						add(senpaiEvil);
						senpaiEvil.alpha = 0;
